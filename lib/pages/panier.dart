...
class _PanierPageState extends State<PanierPage> {
  ...
  void applyPromoCode() async {
    ...
    List<Map<String, dynamic>> coupons = await getCouponsFromFirestore();

    setState(() {
      // Check if the promoCode exists in the coupon list
      Map<String, dynamic>? matchingCoupon = coupons.firstWhere(
      ...
      if (matchingCoupon.isNotEmpty) {
        promoApplied = true;
        discount = matchingCoupon['discount'] is double
            ? matchingCoupon['discount']
            : double.tryParse(matchingCoupon['discount'].toString());;
      } else {
        promoApplied = false;
        discount = 0.0;
      }
  ...
  double get subtotal => products
    ...
    if (product['price'] is String) {
      price = double.tryParse(product['price']) ?? 0.0; // Convert String to double
...
Future<List<Map<String, dynamic>>> getUserCartProducts(String userId) async {
  try {
    ...
    CollectionReference userCartCollection = FirebaseFirestore.instance.collection('users').doc(userId).collection('cart');

    // Fetch all items in the cart
    QuerySnapshot cartSnapshot = await userCartCollection.get();

    // Extract product IDs from the cart
    List<String> productIds = cartSnapshot.docs.map((doc) => doc["id"] as String).toList();
    ...
    QuerySnapshot productsSnapshot = await productsCollection.where(FieldPath.documentId, whereIn: productIds).get();
...
Future<List<Map<String, dynamic>>> getCouponsFromFirestore() async {
  try {
    // Get reference to the Firestore collection "coupon"
    CollectionReference coupons = FirebaseFirestore.instance.collection('coupon');

    // Retrieve all documents from the collection
    QuerySnapshot querySnapshot = await coupons.get();

    // Map each document to a JSON (Map<String, dynamic>)
    List<Map<String, dynamic>> couponList = querySnapshot.docs.map((doc) {
      return {
        'code': doc['code'],
    ...
    // Handle errors
    print('Error retrieving coupons: $e');
    return [];
  }
}
